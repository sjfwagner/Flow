 ...
 * @author sophiewagner
 */
public class NationalFlowOutAnalysis {
    ...
    public NationalFlowOutAnalysis(MyNetwork t, MyNetwork o, MyNode a)
    {
        traffic = t; 
        od = o; 
        target = a; 
    ...
    public void DoAnalysis()
    {
        for (int j = 0; j < od.getN(); j++)
        {
            if(od.getEdges().getEdge(target.getNumber(), j).getStrength() > 0 )
            {
                ...
                od.getEdges().getEdge(target.getNumber(), j).setFlow(ek.getMaxFlowValue().doubleValue());
